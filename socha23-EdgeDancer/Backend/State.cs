        ...
        public string GetFEN()
        {
            ...
            for (int y = 7; y >= 0; y--)
                for (int x = 0; x < 8; x++)
                {
                    ...
                    if (f.Free()) fen += f.fishes.ToString();
                    else
                    if (f.Piece.Team == PlayerTeam.ONE) fen += "r";
                    else
                    if (f.Piece.Team == PlayerTeam.TWO) fen += "b";
                }

            if (CurrentPlayer == PlayerOne) fen += " r "; else fen += " b ";

            fen += PlayerOne.Fishes.ToString() + " ";
            fen += PlayerTwo.Fishes.ToString() + " ";
            fen += Turn.ToString();
