...
Usage: start.sh [options]
    ...
    --help:
        ...
        static void ExecuteCommunationLoop(NetworkStream stream)
        {
            ...
            while (true)
            {
                // Receive text over netowrk and preprocess it
                ...
                var serializer =
 new XmlSerializer(typeof(Received));
        ...
        {
            Console.WriteLine("Entering Program.Send() at " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
            byte[] data = Encoding.ASCII.GetBytes(message);
            stream.Write(data, 0, data.Length);
            if (LogNetwork)
                ConsoleWriteLine($"Sent: {message}", ConsoleColor.Cyan);
            Console.WriteLine("Leaving Program.Send() at " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
        }
        static string Recieve(NetworkStream stream)
        {
            Console.WriteLine("Entering Program.Receive() at " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
            ...
            Console.WriteLine("Leaving Program.Receive() at " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
